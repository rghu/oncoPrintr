...
oncoPrint <- function(df, sort=TRUE, convert = TRUE, total_samples = NA, geneName = NA, annotation = NA, annotation_order = NA, merge_scnas = F, df2 = NA, colors = list(Amplification = "red", Deletion = "blue"), alteration_score = list(Amplification = 5, Deletion = 4, Nonsense = 2.8, Frameshift = 2.5, Splicing = 2.5, InFrame = 2, Promoter = 2, Mutation =1, Missense=1, Present = 1, Absent = 0.5,Hypopharynx=1, Larynx=1, Oral_Cavity=1, Oropharynx=1, Sinonasal=1, Unknown_primary=1, NotTested = 0, None = 0, NotPresent = 0, del = 3, homodel = 2, LOH = 1.5, CNLOH = 1), printSamples = T, xpadding = 0.1, ypadding = 0.1) {
  ...
  remove_duplicates <- function(df){
    choose_alteration_type <- function(x){
      for (i in 1:length(x)){
        ...
        }else if(grepl("Hypopharynx", x[i])){
          return(x[grep("Hypopharynx", x[i]) ])
        }else if(grepl("Larynx", x[i])){
          return(x[grep("Larynx", x[i]) ])
        }else if(grepl("Oral_Cavity", x[i])){
          return(x[grep("Oral_Cavity", x[i]) ])
        }else if(grepl("Oropharynx", x[i])){
          return(x[grep("Oropharynx", x[i]) ])
        }else if(grepl("Sinonasal", x[i])){
          return(x[grep("Sinonasal", x[i]) ])
        }else if(grepl("Unknown_primary", x[i])){
          return(x[grep("Unknown_primary", x[i]) ])
      ...
      for(j in 1:nsamples) {
        ...
        if(!is.na(altered)){ # there is an alteration
          if(grepl("," ,altered)){ # alteration is a mix of two seperated by a comma
            ...
            }else if( altered == "Amplification" || altered == "Deletion" || altered == "Present" || altered == "Absent" || altered == "Hypopharynx"|| altered == "Larynx"|| altered == "Oral_Cavity"|| altered == "Oropharynx"|| altered == "Sinonasal"|| altered == "Unknown_primary"|| altered == "NotPresent" || altered == "NotTested"|| altered == "homodel" || altered == "del" || altered == "CNLOH" || altered == "LOH"){
  ...
  colors[ which(oncoCords[, "altered"] == "Hypopharynx") ] <- "#E69F00"
  colors[ which(oncoCords[, "altered"] == "Larynx") ] <- "#56B4E9"
  colors[ which(oncoCords[, "altered"] == "Oral_Cavity") ] <- "#009E73"
  colors[ which(oncoCords[, "altered"] == "Oropharynx") ] <- "#F0E442"
  colors[ which(oncoCords[, "altered"] == "Sinonasal") ] <- "#0072B2"
  colors[ which(oncoCords[, "altered"] == "Unknown_primary") ] <- "#D55E00"


  
  colors.scna[ which(oncoCords.scna[, "altered"] == "Present") ] <- "darkred"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Absent") ] <- "lightskyblue3"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Hypopharynx") ] <- "#E69F00"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Larynx") ] <- "#56B4E9"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Oral_Cavity") ] <- "#009E73"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Oropharynx") ] <- "#F0E442"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Sinonasal") ] <- "#0072B2"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Unknown_primary") ] <- "#D55E00"
  ...
  if(!is.na(annotation)){
    ...
    legend(x="bottomleft", c(  "Hypopharynx", "Larynx", "Oral_Cavity", "Oropharynx", "Sinonasal", "Unknown_primary"), fill = c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00"), horiz=T, border = F, cex=0.75, bty = 'n', x.intersp = 0.5)
    ...
    legend(x="bottomleft", c(  "Deletion", "Present", "Absent", "Hypopharynx", "Larynx", "Oral_Cavity", "Oropharynx", "Sinonasal", "Unknown_primary"), fill = c('#174D9D', "darkred", "lightskyblue3",  "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00"), horiz=T, border = F, cex=0.75, bty = 'n')
    close.screen(all.screens = TRUE)
  }
  par(def.par) 
  res <- list()
