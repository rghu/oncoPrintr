...
oncoPrint <- function(df, sort=TRUE, convert = TRUE, total_samples = NA, geneName = NA, annotation = NA, annotation_order = NA, merge_scnas = F, df2 = NA, colors = list(Amplification = "red", Deletion = "blue"), alteration_score = list(Amplification = 5, Deletion = 4, Nonsense = 2.8, Frameshift = 2.5, Splicing = 2.5, InFrame = 2, Promoter = 2, Mutation =1, Missense=1, Present = 1, Absent = 0.5, Major_Salivary_Gland = 1, Minor_Salivary_Gland = 1, Tracheobronchial = 1, Cribriform_or_Tubular = 1, Solid =1, Unknown = 1, NotTested = 0, None = 0, NotPresent = 0, del = 3, homodel = 2, LOH = 1.5, CNLOH = 1), printSamples = T, xpadding = 0.1, ypadding = 0.1) {
  ...
  remove_duplicates <- function(df){
    choose_alteration_type <- function(x){
      for (i in 1:length(x)){
        ...
        }else if(grepl("Major_Salivary_Gland", x[i])){
          return(x[grep("Major_Salivary_Gland", x[i]) ])
        }else if(grepl("Minor_Salivary_Gland", x[i])){
          return(x[grep("Minor_Salivary_Gland", x[i]) ])
        }else if(grepl("Tracheobronchial", x[i])){
          return(x[grep("Tracheobronchial", x[i]) ])
        }else if(grepl("Cribriform_or_Tubular", x[i])){
          return(x[grep("Cribriform_or_Tubular", x[i]) ])
        }else if(grepl("Solid", x[i])){
          return(x[grep("Solid", x[i]) ])
      ...
      for(j in 1:nsamples) {
        ...
        if(!is.na(altered)){ # there is an alteration
          if(grepl("," ,altered)){ # alteration is a mix of two seperated by a comma
            ...
            }else if( altered == "Amplification" || altered == "Deletion" || altered == "Present" || altered == "Absent" || altered == "Major_Salivary_Gland" || altered == "Minor_Salivary_Gland" || altered == "Tracheobronchial" || altered == "Cribriform_or_Tubular" || altered == "Solid" || altered ==  "Unknown" || altered == "NotPresent" || altered == "NotTested"|| altered == "homodel" || altered == "del" || altered == "CNLOH" || altered == "LOH"){
  ...
  colors[ which(oncoCords[, "altered"] == "Major_Salivary_Gland") ] <- "#E69F00"
  colors[ which(oncoCords[, "altered"] == "Minor_Salivary_Gland") ] <- "#56B4E9"
  colors[ which(oncoCords[, "altered"] == "Tracheobronchial") ] <- "#009E73"
  colors[ which(oncoCords[, "altered"] == "Cribriform_or_Tubular") ] <- "#F0E442"
  colors[ which(oncoCords[, "altered"] == "Solid") ] <- "#0072B2"
  ...
  colors.scna[ which(oncoCords.scna[, "altered"] == "Major_Salivary_Gland") ] <- "#E69F00"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Minor_Salivary_Gland") ] <- "#56B4E9"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Tracheobronchial") ] <- "#009E73"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Cribriform_or_Tubular") ] <- "#F0E442"
  colors.scna[ which(oncoCords.scna[, "altered"] == "Solid") ] <- "#0072B2"
  ...
  if(!is.na(annotation)){
    ...
    legend(x="bottomleft", c("Amplification", "Deletion", "LOH", "Present", "Absent", "Major_Salivary_Gland", "Minor_Salivary_Gland", "Tracheobronchial", "Cribriform_or_Tubular", "Solid", "Unknown"), fill = c( '#EA2E49', '#174D9D', 'darkkhaki', 'darkorchid2', "orange", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00"), horiz=T, border = F, cex=0.9, bty = 'n', x.intersp = 0.5)
    ...
    screen(2)
    par(mar=c(0,0,0,0))
    legend(x="topleft", c("Missense mutation", "Nonsense mutation", "Truncating mutation", "In-Frame mutation", "Promoter mutation", "Amplification", "Deletion"), fill = c('#26A818', 'black',  '#A05E35', '#F26529', '#2986E2',  '#EA2E49', '#174D9D'), horiz=T, border = F, cex=0.75, bty = 'n')
    legend(x="bottomleft", c(  "Present", "Absent", "Major_Salivary_Gland", "Minor_Salivary_Gland", "Tracheobronchial", "Cribriform_or_Tubular", "Solid", "Unknown"), fill = c("darkred", "lightskyblue3", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00"), horiz=T, border = F, cex=0.75, bty = 'n')
    close.screen(all.screens = TRUE)
  }
  par(def.par) 
  res <- list()
